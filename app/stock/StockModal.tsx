...
import { StockSchema, Size, SizeSchema, Stock } from './StockSchema';

interface Props {
    isOpen: boolean;
    onClose?: () => void;
    onSubmit: (data: Stock) => Promise<Stock>;
    stock: Stock;
}

function StockModal({ isOpen, onClose, onSubmit, stock }: Props) {
    const {
        register,
        control,
        handleSubmit,
        reset,
        formState: { errors },
    } = useForm<Stock>({ resolver: zodResolver(StockSchema), defaultValues: stock });
    ...
    return (
        <>
            <Modal isOpen={isOpen} onClose={onClose} title='Stock Update' >
                <form onSubmit={handleSubmit(data => {
                    onSubmit(data).then(newStock => {
                        reset(newStock)
                    })
                })}>
                    <div>
                        <div className='join'>
                            <div className="join join-item join-vertical w-full px-10">
                                ...
                                <div className='join join-item pt-9'>
                                    <button className='w-100 btn btn-sm  btn-primary mr-9'>Update Stock</button>
                            ...
                            <div>
                                ...
                                <button className='btn btn-sm btn-secondary' type="button" onClick={() => append({ id: 0, sizeQuantity: 0, size: '', stockId: stock.id })}>
                                    Add Size
                                </button>
